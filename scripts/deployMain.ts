...
import Main from "../wrappers/Main"; // this is the interface class from step 7
dotenv.config(); 
...
export async function run() {
  ...
  const code = Cell.fromBoc(fs.readFileSync("build/main.cell"))[0];
  const mnemonic = process.env.MNEMONIC!;
  const key = await mnemonicToWalletKey(mnemonic.split(" "));
  const main = Main.createForDeploy(code, {
    seqno: 0,
