"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.openMockContext = exports.openContext = exports.getRawAST = void 0;
const context_1 = require("../context");
const grammar_1 = require("./grammar");
const store = (0, context_1.createContextStore)();
function getRawAST(ctx) {
    const r = store.get(ctx, "types");
    if (!r) {
        throw Error("No AST found in context");
    }
    return r;
}
exports.getRawAST = getRawAST;
function openContext(ctx, sources, funcSources) {
    const asts = sources.map((source) => (0, grammar_1.parse)(source.code, source.path, source.origin));
    const types = [];
    const functions = [];
    const constants = [];
    for (const a of asts) {
        for (const e of a.entries) {
            if (e.kind === "def_struct" ||
                e.kind === "def_contract" ||
                e.kind === "def_trait" ||
                e.kind === "primitive") {
                types.push(e);
            }
            else if (e.kind === "def_function" ||
                e.kind === "def_native_function") {
                functions.push(e);
            }
            else if (e.kind === "def_constant") {
                constants.push(e);
            }
        }
    }
    ctx = store.set(ctx, "types", {
        sources,
        funcSources,
        functions,
        constants,
        types,
    });
    return ctx;
}
exports.openContext = openContext;
// Creates a mock context with the given AST elements needed for testing
// purposes
function openMockContext(ctx, types, functions, constants) {
    const sources = [];
    const funcSources = [];
    ctx = store.set(ctx, "types", {
        sources,
        funcSources,
        functions,
        constants,
        types,
    });
    return ctx;
}
exports.openMockContext = openMockContext;
